...
func read(lex *lexer, v reflect.Value) {
	...
	case scanner.Float:
		...
		case reflect.Float32:
			f, _ := strconv.ParseFloat(lex.text(), 32) // NOTE: ignoring errors
			v.SetFloat(f)
		case reflect.Float64:
			f, _ := strconv.ParseFloat(lex.text(), 64) // NOTE: ignoring errors
			v.SetFloat(f)
		default:
			panic(fmt.Sprintf("unexpected type %q", v.Kind()))
		}
		lex.next()
		return
	case '#': // ex. #C(1.0 2.0)
		...
		lex.next() // consume '('
		r := lex.text()
		lex.next() // consume `real`
		i := lex.text()
		lex.next() // consume `imag`
		lex.next() // consume ')'
		...
		real, _ := strconv.ParseFloat(r, bitSize) // NOTE: ignoring errors
		imag, _ := strconv.ParseFloat(i, bitSize) // NOTE: ignoring errors
...
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Struct: // ((name value) ...)
		...
		for !endList(lex) {
			...
			name := lex.text()
			lex.next()
			read(lex, v.FieldByName(tags[name]))
			lex.consume(')')
		}
	...
	case reflect.Interface: // ("type" (value ...))
		...
		case "[]int":
			var x []int
			t = reflect.TypeOf(x)
		default: // todo: 現状はすべての型に対応できない
			panic(fmt.Sprintf("cannot decode type of interface %q", token))
		}
