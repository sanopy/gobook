...
func Example_map2() {
	...
	Display("map", map[Person]int{
		{"Alice", 20}: 1,
		{"Bob", 25}:   2,
	})
	// Output:
	// Display map (map[display.Person]int):
	// map[{Name: "Alice", Age: 20}] = 1
	// map[{Name: "Bob", Age: 25}] = 2
}

func Example_map3() {
	Display("map", map[[2]string]int{
		{"Hello", "World"}: 1,
	})
	// Output:
	// Display map (map[[2]string]int):
