...
var funcType = flag.String("f", "", "specify function ('egg', 'mogul' or 'saddle')")
...
func main() {
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			ax, ay, az, isAFinite := corner(i+1, j)
			bx, by, bz, isBFinite := corner(i, j)
			cx, cy, cz, isCFinite := corner(i, j+1)
			dx, dy, dz, isDFinite := corner(i+1, j+1)
